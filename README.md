...
contract/                     Sui Move package (the on-chain half)
  sources/pharma_track.move   Batch (with stake collateral), Checkpoint, HoldRecord, AdminRegistry, RegulatorRegistry, ManufacturerRegistry, Unit
  tests/pharma_track_tests.move  50 unit tests (test with `sui move test`)
...
| `AdminRegistry` + `RegulatorRegistry` + `ManufacturerRegistry` (Move) | All three are shared allow-lists (`VecSet<address>`) — `RegulatorRegistry` gates who can place/release holds, `ManufacturerRegistry` gates who can call `create_batch`, `AdminRegistry` gates who can manage *either* of those two, all checked by address, not by object possession. `admin_add_admin`/`admin_remove_admin` manage `AdminRegistry` itself, and refuse to remove the last remaining admin. This replaced an earlier bearer-`AdminCap`-object design specifically because losing that one object (or it leaking) would have permanently locked out every future admin action — an allow-list can be backed up ahead of time and can't be emptied into that same unrecoverable state |
...
| `Batch.stake` (Move) | Optional `Balance<SUI>` collateral posted at `create_batch`, locked for the batch's whole shelf life. `place_hold` slashes it to the placing regulator the instant a Critical + Counterfeit hold lands (a paid bounty, not a treasury burn); `withdraw_stake` lets the manufacturer reclaim it, but only once `expiry_ms` has passed with no such hold and the batch isn't currently held |
| `report_suspicion` (Move) | Permissionless — no registry check, same trust model as `add_checkpoint`. Emits a `SuspicionReported` event with the reporter's address and a note; doesn't touch any batch state, it's a public tip a regulator can act on, not a verdict |
| Register tab | Listed manufacturer calls `create_batch` with an expiry date and an optional SUI stake; also generates the printable per-item verify QR sheet; "Admin: manage manufacturer access" and "Admin: manage admin access" panels appear for any listed admin |
| Scan tab | Distributor/pharmacy calls `add_checkpoint` |
| Verify tab | Public, no wallet needed — reads the object straight from Sui, shows the ledger + hold history + expiry + stake status, runs the single-batch and cross-batch AI checks, lists/accepts suspicion reports, and can also generate item QR codes for an existing batch. Scanning a printed item QR (with a serial) whose batch is currently held shows a loud "do not use this medicine" banner, even though that specific package's QR was printed before the hold existed |
...
sui move test              # 50 tests should pass
sui client publish --gas-budget 200000000
```

Copy the **Package ID** from the "Published Objects" section of the output, and all three shared registry object IDs from "Created Objects" (types `...::batch::RegulatorRegistry`, `...::batch::ManufacturerRegistry`, and `...::batch::AdminRegistry`, all `Owner: Shared`) — unlike the package ID, a shared object's ID isn't derivable from anything else, it only appears in this output.
...
1. **Register batch** (listed manufacturer) — enter a batch code, product name, expiry date, and an optional SUI stake, sign. You get a QR encoding a public verify link, and can immediately generate a sheet of individually-numbered per-item QR codes to print. If the connected wallet is a listed admin, "Admin: manage manufacturer access" and "Admin: manage admin access" panels appear — add or revoke a manufacturer address, or add a backup admin, here.
...
3. **Verify a product** (anyone, no wallet needed) — paste/scan a batch ID (or a printed item QR, which shows its package number too). Shows the full checkpoint ledger, hold history, and expiry (with a warning banner inside a 60-day "expires soon" window, and a hard stop once actually expired). From here, a listed regulator can place/release holds, and anyone can run the AI check or generate more item QR codes.
   ...
   - Anyone can submit a suspicion report with a short note, and see every report already submitted for this batch — no wallet allow-listing required.
...
```bash
cd contract && sui move test          # 50 unit tests, covers happy paths + every abort condition
...
- **Stake is a counterfeit-specific bond, slashed as a bounty, not burned.** `place_hold` only touches `Batch.stake` for the exact severity+category combination (Critical + Counterfeit) that represents a real, adjudicated claim against the manufacturer's collateral — a Critical cold-chain-breach hold, for instance, leaves the stake untouched, since that's not what the bond was posted against. Sending the slashed amount to the regulator who placed the hold (rather than burning it or routing it to a treasury) turns catching counterfeits into something with a direct financial reward, not just unpaid diligence.
...
- **Suspicion reports are as permissionless as checkpoints, for the same reason.** `report_suspicion` doesn't check any registry — restricting who can raise a tip would just add friction to the exact signal a real recall often starts with: an unverified public report, before any regulator has looked at anything. It's explicitly not a verdict (nothing freezes), just a visible signal a regulator can choose to act on.
...
- **`AdminRegistry` has no timelock or per-action multisig.** Any single listed admin can unilaterally add/revoke a regulator, manufacturer, or even another admin — the registry protects against *losing* admin access entirely, not against one admin acting alone. A real deployment might want N-of-M approval for admin actions themselves, the same way Critical holds require two regulators to release.
...
- **Stake is denominated only in SUI, and the slash is all-or-nothing.** There's no partial slashing scaled to severity of the finding, no multi-token support, and no way to top up a stake after registration — `Batch.stake` is set once at `create_batch` and only ever fully emptied (slashed or withdrawn), never adjusted. A real deployment would likely want graduated penalties and the ability to add more collateral later.
- **Suspicion reports have no anti-spam cost.** Anyone can submit unlimited reports for free, so a malicious actor could flood a legitimate batch with noise to bury a real signal, or spam the event log generally. A real deployment might require a small bonded fee (refunded if a regulator later confirms the report) or basic rate-limiting.
