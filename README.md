...
9. Admin access (`AdminRegistry`, which manages the regulator, manufacturer, and pharmacy allow-lists) is itself a revocable, multi-member allow-list rather than a single bearer capability object — any listed admin can seed a backup admin ahead of time via `admin_add_admin`, and the last remaining admin can never be removed, closing off the single-point-of-failure a lone `AdminCap` used to be. Once a second admin exists, sensitive admin actions require **propose + confirm from two different admins**, the same two-signer pattern as releasing a Critical hold — a lone admin can no longer unilaterally add/revoke a regulator, manufacturer, pharmacy, or fellow admin.
...
contract/                     Sui Move package (the on-chain half)
  sources/pharma_track.move   Batch (with stake collateral), Checkpoint, HoldRecord, AdminRegistry, RegulatorRegistry, ManufacturerRegistry, PharmacyRegistry, Unit, SuspicionReport
  tests/pharma_track_tests.move  56 unit tests (test with `sui move test`)
...
frontend/                     React + Vite app (the off-chain half)
  ...
  src/
    lib/                       network config, Sui object parsing, Gonka integration (single- and cross-batch), rule-based checks, QR helpers, event aggregation, allow-list hooks, toast state, Explorer URL helpers
    components/                Register / Scan / Verify / Mint / Pay / Active-Holds / Stats screens, QR generation + scanning, hold controls, stake + suspicion-report panels, admin panels, AI report panels, error boundary, copy-able code chips
```
...
| `AdminRegistry` + `RegulatorRegistry` + `ManufacturerRegistry` + `PharmacyRegistry` (Move) | All four are shared allow-lists (`VecSet<address>`) — `RegulatorRegistry` gates who can place/release holds, `ManufacturerRegistry` gates who can call `create_batch`, `PharmacyRegistry` gates who can call `mint_unit`, `AdminRegistry` gates who can manage the other three, all checked by address, not by object possession. `admin_add_admin` always works single-signer (bootstrapping a second admin can't itself require a second admin); once 2+ admins exist, every other `admin_*` action requires `propose_admin_action` + `confirm_admin_action` from two *different* admins instead. This replaced an earlier bearer-`AdminCap`-object design specifically because losing that one object (or it leaking) would have permanently locked out every future admin action — an allow-list can be backed up ahead of time and can't be emptied into that same unrecoverable state |
| Hold system | `place_hold` requires a severity (Advisory / Recall / Critical), a category (Counterfeit / Quality Defect / Labeling Error / Cold-Chain Breach / Other), a mandatory case reference, and freezes `add_checkpoint`, `mint_unit`, and `purchase_and_burn`. `release_hold` requires a mandatory release note; **Critical** holds reject `release_hold` outright and require `propose_release` + `confirm_release` from two *different* listed regulators. `escalate_stale_hold` — callable by anyone — permanently flags a hold that's sat past its severity-scaled review window, without releasing it or changing what it blocks. Every cycle is kept forever in `hold_history`, even after release, including who proposed vs. who confirmed and whether it was ever escalated |
| `Unit` (Move) | A single-use, shared "sale ticket" for one physical package. `mint_unit` (gated by `PharmacyRegistry`) creates it with a price and a 10-minute expiry (aborts if the batch is held or already expired); `purchase_and_burn` takes the `Batch` too, re-checks it isn't held or expired *at redemption time* (not just at mint time), takes exact payment, forwards it to the manufacturer, and **deletes the object** — the QR pointing at it becomes permanently unredeemable, which is what makes it single-use (not a flag that could be bypassed, an object that stops existing) |
| `Batch.stake` (Move) | Optional `Balance<SUI>` collateral posted at `create_batch`, locked for the batch's whole shelf life. `place_hold` slashes some or all of it to the placing regulator via `slash_percent` — 100% for Critical + Counterfeit, 50% for a Critical Quality Defect/Cold-Chain Breach, 25% for a Recall + Counterfeit, 0% otherwise — as a paid bounty, not a treasury burn; `withdraw_stake` lets the manufacturer reclaim whatever's left, but only once `expiry_ms` has passed and the batch isn't currently held |
| `SuspicionReport` (Move) | A shared object created by `report_suspicion` (permissionless, same trust model as `add_checkpoint`), holding a bonded `Coin<SUI>` from the reporter. A listed regulator later calls `confirm_suspicion` (bond refunded) or `reject_suspicion` (bond forfeited to them) to consume it — the bond is what keeps the public report feed from being flooded for free |
| Register tab | Listed manufacturer calls `create_batch` with an expiry date and an optional SUI stake; also generates the printable per-item verify QR sheet; "Admin: manage manufacturer access", "Admin: manage admin access", and a two-signer admin-action panel appear for any listed admin |
...
| Create sale QR tab | Listed pharmacy calls `mint_unit` at checkout; disabled with a clear warning if the batch is on hold, expired, or the connected wallet isn't a listed pharmacy; an "Admin: manage pharmacy access" panel appears for any listed admin |
...
| `src/lib/registry.ts` | `useIsListed` hook shared by all three registries — reads a `VecSet<address>` field directly rather than calling into Move for what's just a field read |
...
sui move test              # 56 tests should pass
sui client publish --gas-budget 200000000
```

Copy the **Package ID** from the "Published Objects" section of the output, and all four shared registry object IDs from "Created Objects" (types `...::batch::RegulatorRegistry`, `...::batch::ManufacturerRegistry`, `...::batch::AdminRegistry`, and `...::batch::PharmacyRegistry`, all `Owner: Shared`) — unlike the package ID, a shared object's ID isn't derivable from anything else, it only appears in this output.
...
VITE_ADMIN_REGISTRY_OBJECT_ID=<the AdminRegistry object id from the same publish>
VITE_PHARMACY_REGISTRY_OBJECT_ID=<the PharmacyRegistry object id from the same publish>
...
1. **Register batch** (listed manufacturer) — enter a batch code, product name, expiry date, and an optional SUI stake, sign. You get a QR encoding a public verify link, and can immediately generate a sheet of individually-numbered per-item QR codes to print. If the connected wallet is a listed admin, "Admin: manage manufacturer access" and "Admin: manage admin access" panels appear (add/revoke a manufacturer, or add a backup admin), plus a two-signer admin-action panel once a second admin exists — propose an action there, then a *different* admin confirms it.
...
3. **Verify a product** (anyone, no wallet needed) — paste/scan a batch ID (or a printed item QR, which shows its package number too). Shows the full checkpoint ledger, hold history, and expiry (with a warning banner inside a 60-day "expires soon" window, and a hard stop once actually expired). From here, a listed regulator can place/release holds, and anyone can run the AI check or generate more item QR codes.
   ...
   - If the connected wallet is a listed admin, an "Admin: manage regulator access" panel appears — add or revoke a regulator address here.
   - If the batch has a stake, a panel shows how much is locked; the manufacturer can withdraw it once the batch has expired with no counterfeit hold and isn't currently held.
   - Anyone can submit a suspicion report with a short note and a small SUI bond, and see every report already submitted for this batch — no wallet allow-listing required. A listed regulator can confirm an open report (bond refunded to the reporter) or reject it as spam (bond forfeited to the regulator).
...
```bash
cd contract && sui move test          # 56 unit tests, covers happy paths + every abort condition
...
- **Critical holds need two different signers to release.** One person can place a critical hold alone (pulling the emergency brake shouldn't require consensus), but releasing one requires `propose_release` from one listed regulator and `confirm_release` from a *different* one — mirrors how real recalls aren't unilateral decisions. Advisory/Recall holds stay single-signer, since requiring two people for every minor hold would just create friction without a matching real-world norm.
- **Stale-hold escalation is permissionless by design, same reasoning as `add_checkpoint`.** `escalate_stale_hold` doesn't check the caller against any registry — it only turns an already-public, independently-computable fact ("this hold has been open longer than its review window") into a permanent on-chain record and event. Restricting who can flag that would just add friction to something anyone can already verify by reading `held_at_ms` and the current clock themselves.
...
- **Allow-list over bearer capability, all the way up, including for the admin role itself.** An earlier version gated holds with a `RegulatorCap` object — mint one, anyone holding it can act, forever, with no way to revoke it once handed out. Switching regulator/manufacturer access to address allow-lists closed that, but for a while the admin role that managed *those* lists was still a single bearer `AdminCap` — the exact same unrecoverable-loss problem, just one level up: lose that one object and nobody can ever change either registry again. `AdminRegistry` closes that too: it's an allow-list like the other two, any listed admin can seed a backup via `admin_add_admin` *before* ever needing one, and `admin_remove_admin` refuses to strip out the last remaining admin.
...
- **Stake is slashed as a bounty, not burned.** Sending the slashed amount to the regulator who placed the hold (rather than burning it or routing it to a treasury) turns catching counterfeits into something with a direct financial reward, not just unpaid diligence.
...
- **Suspicion reports are bonded, not free.** `report_suspicion` requires a nonzero `Coin<SUI>` bond, held in a shared `SuspicionReport` object until a regulator confirms it (bond refunded) or rejects it (bond forfeited to the regulator, compensating their time reviewing something that turned out to be noise). A completely free report feed — the original design — has no cost to flooding it with spam; a bond, even a small one, makes that an active choice with a price rather than a free action.
- **`mint_unit` gated by a `PharmacyRegistry`, closing a gap that existed since the very first version.** Manufacturer and regulator access were vetted allow-lists from early on; "pharmacy" wasn't, meaning any wallet could mint a sale QR against any batch — since payment always routes to the batch's manufacturer regardless of who mints, this wasn't a way to steal funds, but it was a way to create confusing, unauthorized sale flows for a product with no real seller behind them. Same allow-list pattern as the other two roles closes it.
- **Admin actions become two-signer once a second admin exists, mirroring Critical hold release.** A lone `AdminRegistry` admin acting unilaterally is fine when they're the *only* admin (there's nobody else to check them, and requiring confirmation would just deadlock a fresh deployment), but once a backup admin has been added specifically to avoid a single point of failure, that backup should also mean no single admin can unilaterally add/revoke a regulator, manufacturer, pharmacy, or fellow admin. `propose_admin_action`/`confirm_admin_action` enforce that with a single pending-action slot per `AdminRegistry`, the same "one thing in flight at a time" shape as `Batch.pending_release_by`. `admin_add_admin` is the one exception, kept always-direct, since requiring a second admin's confirmation to add the *first* backup admin would make it impossible to ever get past one admin at all.
...
- **Stake is denominated only in SUI, and there's no way to top up after registration.** `Batch.stake` is set once at `create_batch` — a manufacturer can't add more collateral later if circumstances change, and there's no multi-token support. Slashing itself is now graduated (see the design decision above), but the stake amount itself is fixed at registration time.
- **Suspicion report bonds have no minimum.** `report_suspicion` only requires a nonzero amount — a reporter can post a bond of 1 MIST, which is a real cost in name only. A production version would likely want a configurable minimum bond (scaled to, say, the batch's stake or a fixed floor) to make the anti-spam deterrent meaningful rather than symbolic.
- **Suspicion confirm/reject relies on any listed regulator's judgment call, with no appeal path.** There's no dispute mechanism if a reporter believes a rejection was made in bad faith or without real review — a regulator's decision is final and immediate. A real deployment might want a cooldown, a second-regulator sign-off for rejections specifically, or an appeal window.
- **`AdminRegistry`'s two-signer requirement covers add/revoke actions, not `admin_add_admin` itself.** Adding a backup admin always stays single-signer by design (bootstrapping problem — see the design decision above), which means any single existing admin can unilaterally grow the admin set even once others exist. A real deployment might want to at least log/notify on every `admin_add_admin` call given it's the one action that bypasses the two-signer requirement.
- **Cross-batch check is capped by `multiGetObjects`'s per-call object limit.** Sui's JSON-RPC caps a single `multiGetObjects` call at 50 objects; a manufacturer with more than 50 registered batches would need pagination added to `CrossBatchPanel` to compare all of them, not just the first 50 found.
