...
contract/                     Sui Move package (the on-chain half)
  sources/pharma_track.move   Batch, Checkpoint, HoldRecord, AdminRegistry, RegulatorRegistry, ManufacturerRegistry, Unit
  tests/pharma_track_tests.move  42 unit tests (test with `sui move test`)
...
| Register tab | Listed manufacturer calls `create_batch` with an expiry date; also generates the printable per-item verify QR sheet; "Admin: manage manufacturer access" and "Admin: manage admin access" panels appear for any listed admin |
...
sui move test              # 42 tests should pass
...
1. **Register batch** (listed manufacturer) — enter a batch code, product name, and expiry date, sign. You get a QR encoding a public verify link, and can immediately generate a sheet of individually-numbered per-item QR codes to print. If the connected wallet is a listed admin, "Admin: manage manufacturer access" and "Admin: manage admin access" panels appear — add or revoke a manufacturer address, or add a backup admin, here.
...
```bash
cd contract && sui move test          # 42 unit tests, covers happy paths + every abort condition
